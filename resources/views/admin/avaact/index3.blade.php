...
@section('scripts')
    <script>
        ...
        function actualizarTablaRezago(sel) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    const filtros = data.filtros ?? null;
                    if (filtros) {
                        // Limpiar y llenar estrategias
                        $('#id_e').empty().append('<option value="0">Todas</option>');
                        filtros.estrategias.forEach(e => {
                            if (sel === 'id_e' && e.id == id_est) {
                                $('#id_e').append('<option value="' + e.id + '" selected>' + e.codigo_e + '. ' + e.nombre_e + '</option>');
                            } else {
                                $('#id_e').append('<option value="' + e.id + '">' + e.codigo_e + '. ' + e.nombre_e + '</option>');
                            }
                        });

                        // Limpiar y llenar dimensiones
                        $('#id_dim').empty().append('<option value="0">Todas</option>');
                        filtros.dimensiones.forEach(d => {
                            if (sel === 'id_dim' && d.id == id_dim) {
                                $('#id_dim').append('<option value="' + d.id + '" selected>' + d.codigo_d + '. ' + d.nombre_d + '</option>');
                                $('#id_e').val(d.id_e).trigger('change');
                            } else {
                                $('#id_dim').append('<option value="' + d.id + '">' + d.codigo_d + '. ' + d.nombre_d + '</option>');
                            }
                        });

                        // Limpiar y llenar apuestas
                        $('#id_a').empty().append('<option value="0">Todas</option>');
                        filtros.apuestas.forEach(a => {
                            if (sel === 'id_a' && a.id == id_apu) {
                                $('#id_a').append('<option value="' + a.id + '" selected>' + a.codigo_a + '. ' + a.nombre_a + '</option>');
                                $('#id_dim').val(a.id_d).trigger('change');
                            } 
                            else {
                                $('#id_a').append('<option value="' + a.id + '">' + a.codigo_a + '. ' + a.nombre_a + '</option>');
                            }
                        });
                    }
        ...
        $('#id_e, #id_dim, #id_a').on('change', function() {
            sel_actual = this.id;
            // Evitar que se actualice la tabla si el select no ha cambiado
